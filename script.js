...
// Экспорт всех проектов в JSON
function exportAllProjects() {
    try {
        // Сохраняем текущий проект перед экспортом
        saveCurrentProject();
        
        // Получаем все проекты из localStorage
        const allProjects = {};
        for (let key in localStorage) {
            if (key.startsWith('codepen-project-')) {
                allProjects[key] = localStorage.getItem(key);
            }
        }
        
        // Также экспортируем настройки
        const settings = {
            theme: localStorage.getItem('codepen-theme'),
            globalTheme: localStorage.getItem('codepen-global-theme'),
            library: localStorage.getItem('codepen-library'),
            ignoreAlerts: localStorage.getItem('codepen-ignore-alerts'),
            injectThemeCSS: localStorage.getItem('codepen-inject-theme-css'),
            currentProject: localStorage.getItem('codepen-current-project')
        };
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 800px;">
                <div class="modal-header">
                    <h2>📤 Экспорт всех проектов</h2>
                    <button class="close-btn" onclick="this.closest('.modal').remove()">✕</button>
                </div>
                <div class="modal-body">
                    <div class="settings-section">
                        <p>Скопируйте этот код и сохраните его. Вы сможете импортировать все проекты на другом устройстве:</p>
                        <textarea id="export-code" rows="15" style="width: 100%; font-family: monospace; font-size: 12px;">${jsonString}</textarea>
                        <div class="setting-item project-buttons">
                            <button class="settings-btn success" onclick="
                                document.getElementById('export-code').select();
                                document.execCommand('copy');
                                alert('Код скопирован в буфер обмена!');
                            ">📋 Копировать код</button>
                            <button class="settings-btn primary" onclick="
                                const text = document.getElementById('export-code').value;
                                const blob = new Blob([text], { type: 'application/json' });
                                const url = URL.createObjectURL(blob);
                                const a = document.createElement('a');
                                a.href = url;
                                a.download = 'codepen-projects-' + new Date().toISOString().split('T')[0] + '.json';
                                a.click();
                                URL.revokeObjectURL(url);
                            ">💾 Скачать файл</button>
                        </div>
        ...
        showToast('Экспорт завершен! Всего проектов: ' + Object.keys(allProjects).length, 'success');
    } catch (error) {
        showToast('Ошибка экспорта: ' + error.message, 'error');
    }
}

// Импорт всех проектов из JSON
function importAllProjects() {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 800px;">
            <div class="modal-header">
                <h2>📥 Импорт всех проектов</h2>
                <button class="close-btn" onclick="this.closest('.modal').remove()">✕</button>
            </div>
            <div class="modal-body">
                <div class="settings-section">
                    <p>Вставьте код экспорта или загрузите файл:</p>
                    <textarea id="import-code" rows="15" placeholder="Вставьте JSON код здесь..." style="width: 100%; font-family: monospace; font-size: 12px;"></textarea>
                    <div class="setting-item project-buttons">
                        <button class="settings-btn primary" onclick="document.getElementById('import-file-input').click()">📁 Загрузить файл</button>
                        <button class="settings-btn success" id="import-execute-btn">✅ Импортировать</button>
                        <button class="settings-btn danger" onclick="this.closest('.modal').remove()">❌ Отмена</button>
                    </div>
    ...
    modal.querySelector('#import-file-input').addEventListener('change', (e) => {
        ...
        if (file) {
            ...
            reader.onload = (event) => {
                modal.querySelector('#import-code').value = event.target.result;
            };
    ...
    modal.querySelector('#import-execute-btn').addEventListener('click', () => {
        try {
            const jsonString = modal.querySelector('#import-code').value.trim();
            if (!jsonString) {
                showToast('Вставьте код для импорта', 'error');
                return;
            }
            ...
            // Подтверждение
            if (!confirm(`Импортировать ${Object.keys(importData.projects).length} проектов? Это перезапишет существующие проекты с такими же именами.`)) {
                return;
            }
            
            // Импортируем проекты
            for (let key in importData.projects) {
                localStorage.setItem(key, importData.projects[key]);
            }
            
            // Импортируем настройки (опционально)
            if (importData.settings) {
                if (confirm('Также импортировать настройки (тема, библиотеки)?')) {
                    for (let key in importData.settings) {
                        if (importData.settings[key]) {
                            localStorage.setItem('codepen-' + key, importData.settings[key]);
                        }
                    }
                }
            }
            
            showToast('Импорт завершен! Перезагрузите страницу.', 'success');
            modal.remove();
...
function initializeEventListeners() {
    ...
    // Export/Import all projects buttons
    document.getElementById('modal-export-all-btn').addEventListener('click', exportAllProjects);
    document.getElementById('modal-import-all-btn').addEventListener('click', importAllProjects);
